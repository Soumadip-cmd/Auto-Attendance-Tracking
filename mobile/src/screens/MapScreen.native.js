...
import { View, Text, StyleSheet, TouchableOpacity, ActivityIndicator, Alert, Animated, Dimensions, Platform, PermissionsAndroid, ScrollView, NativeModules } from 'react-native';
...
const withTimeout = (promise, ms, message = 'Request timed out') => Promise.race([promise, new Promise((_, reject) => setTimeout(() => reject(new Error(message)), ms))]);
const normalizeGeofenceList = response => {
  const list = response?.data?.data || response?.data || response?.geofences || [];
  return Array.isArray(list) ? list.filter(g => g?.center?.coordinates?.length === 2 && Number.isFinite(Number(g.center.coordinates[0])) && Number.isFinite(Number(g.center.coordinates[1]))) : [];
};
...
const getExpoGoogleMapsKey = () => Constants.expoConfig?.extra?.GOOGLE_MAPS_API_KEY || Constants.expoConfig?.android?.config?.googleMaps?.apiKey || Constants.manifest2?.extra?.expoClient?.extra?.GOOGLE_MAPS_API_KEY || Constants.manifest2?.extra?.expoClient?.android?.config?.googleMaps?.apiKey || null;
const logGoogleMapsDiagnostics = async () => {
  const expoKey = getExpoGoogleMapsKey();
  let nativeConfig = null;
  try {
    nativeConfig = MapsDiagnostics?.getNativeConfig ? await MapsDiagnostics.getNativeConfig() : {
...
export default function MapScreen() {
  ...
  useEffect(() => {
    ...
    mapTileTimerRef.current = setTimeout(() => {
      ...
      showStatusPopup('warning', 'Using OpenStreetMap', 'Google Maps tiles unavailable. Register your SHA-1 in Google Cloud Console.', true);
    }, MAP_TILE_TIMEOUT_MS);
    initializeLiveMap();
    loadGeofences();
  ...
  useEffect(() => {
    const sub = AndroidGeofencing.addWalkingListener(event => {
      if (event.transition === 'ENTER') setWalkingState(event.activity);else if (event.transition === 'EXIT' && event.activity === walkingState) setWalkingState('STILL');
    });
    return () => sub.remove();
  }, [walkingState]);
  ...
  useEffect(() => {
    ...
    const newHeading = gpsHeading ?? (prev && dist >= MIN_MOVE_METERS ? getBearing(prev.latitude, prev.longitude, newCoord.latitude, newCoord.longitude) : heading);
  ...
  const activityIcon = walkingState === 'WALKING' || walkingState === 'ON_FOOT' ? 'walk' : walkingState === 'RUNNING' ? 'fitness' : walkingState === 'IN_VEHICLE' ? 'car' : walkingState === 'ON_BICYCLE' ? 'bicycle' : 'pause-circle';
  const activityLabel = walkingState === 'WALKING' ? 'Walking' : walkingState === 'RUNNING' ? 'Running' : walkingState === 'ON_FOOT' ? 'On foot' : walkingState === 'IN_VEHICLE' ? 'In vehicle' : walkingState === 'ON_BICYCLE' ? 'Cycling' : 'Still';
  return <View style={styles.container}>
      {/* ── MAP ─────────────────────────────────────────────────────────── */}
      <MapView key={mapInstanceKey} ref={mapRef} provider={PROVIDER_GOOGLE} style={styles.map} showsUserLocation={false} showsMyLocationButton={false} showsCompass={false} showsTraffic={false} mapType={activeMapType} loadingEnabled={false} initialRegion={DEFAULT_REGION} onMapReady={onMapReady} onMapLoaded={onMapLoaded} onPanDrag={() => {
      isFollowingRef.current = false;
    }}>
        {useOsmFallback && <UrlTile urlTemplate={OSM_TILE_URL} tileSize={256} minimumZ={0} maximumZ={19} maximumNativeZ={19} opacity={1} zIndex={100} />}

        {locationHistory.length > 1 && <Polyline coordinates={locationHistory} strokeColor="#6366f1" strokeWidth={4} strokeColors={locationHistory.map((_, i) => `rgba(99,102,241,${0.25 + 0.75 * (i / locationHistory.length)})`)} lineJoin="round" lineCap="round" />}
        ...
        return <React.Fragment key={g._id}>
              <Circle center={{
            latitude: lat,
            longitude: lon
          }} radius={g.radius} fillColor={inside ? 'rgba(16,185,129,0.14)' : 'rgba(99,102,241,0.10)'} strokeColor={inside ? '#10b981' : '#6366f1'} strokeWidth={inside ? 2.5 : 1.5} />
              <Marker coordinate={{
            latitude: lat,
            longitude: lon
          }} title={g.name} description={`${inside ? 'You are inside' : d.toFixed(0) + 'm away'} · Radius ${g.radius}m`} anchor={{
            x: 0.5,
            y: 0.5
          }}>
                <View style={[styles.buildingMarker, {
              backgroundColor: inside ? '#10b981' : '#6366f1'
            }]}>
        ...
        {location && nearestGeofence && !isInside && <Polyline coordinates={[{
        latitude: location.latitude,
        longitude: location.longitude
      }, {
        latitude: nearestGeofence.center.coordinates[1],
        longitude: nearestGeofence.center.coordinates[0]
      }]} strokeColor="#ef4444" strokeWidth={1.5} lineDashPattern={[10, 8]} />}

        {location && accuracy !== null && accuracy < 200 && <Circle center={{
        latitude: location.latitude,
        longitude: location.longitude
      }} radius={Math.max(accuracy, 8)} fillColor="rgba(99,102,241,0.08)" strokeColor="rgba(99,102,241,0.28)" strokeWidth={1} />}

        {markerCoord && <Marker ref={markerRef} coordinate={markerCoord} anchor={{
        x: 0.5,
        y: 0.5
      }} flat={true} rotation={heading} tracksViewChanges={tracksViewChanges} zIndex={10}>
      ...
      {!markerCoord && <View style={styles.pulseDotContainer} pointerEvents="none">
          <Animated.View style={[styles.pulseDotRing, {
        transform: [{
          scale: pulseAnim
        }]
      }]} />
      ...
      backgroundColor: theme.dark ? 'rgba(15,18,30,0.96)' : 'rgba(255,255,255,0.96)'
    }]}>
        <View style={styles.headerTop}>
          ...
          {geofencesLoading && <View style={styles.loadingBadge}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.pillsScroll}>
          {/* Map type pill */}
          <Pill icon={useOsmFallback ? 'map-outline' : isMapReady ? 'map' : 'hourglass'} label={useOsmFallback ? 'OSM map' : mapTilesLoaded ? 'Google Maps' : isMapReady ? 'Map ready' : 'Loading…'} color={useOsmFallback ? '#b45309' : isMapReady ? '#4f46e5' : '#6b7280'} bg={useOsmFallback ? '#fef3c7' : isMapReady ? '#eef2ff' : '#f3f4f6'} />
          {markerCoord && <Pill icon="navigate" label={accuracy !== null ? `GPS ${Math.round(accuracy)}m` : 'GPS live'} color="#4f46e5" bg="#eef2ff" />}
          {locationHistory.length > 1 && <Pill icon="git-branch" label={`Trail ${locationHistory.length}`} color="#7c3aed" bg="#ede9fe" />}
          {nearestGeofence && <Pill icon={isInside ? 'checkmark-circle' : 'close-circle'} label={isInside ? 'Inside zone' : `${Math.round(distance ?? 0)}m away`} color={isInside ? '#16a34a' : '#dc2626'} bg={isInside ? '#f0fdf4' : '#fef2f2'} />}
          {walkingState !== 'UNKNOWN' && <Pill icon={activityIcon} label={activityLabel} color={walkingState === 'STILL' ? '#9ca3af' : '#2563eb'} bg={walkingState === 'STILL' ? '#f3f4f6' : '#dbeafe'} />}
          {speedKmh > 1 && <Pill icon="speedometer" label={`${speedKmh} km/h`} color="#7c3aed" bg="#ede9fe" />}
        </ScrollView>
      </View>

      {/* ── STATUS TOAST ─────────────────────────────────────────────── */}
      {statusPopup && <Animated.View style={[styles.toast, {
      ...
      transform: [{
        translateY: toastAnim.interpolate({
          ...
          <Ionicons name={statusMeta.icon} size={17} color={statusMeta.color} style={{
        marginTop: 1
      }} />
          ...
          <TouchableOpacity onPress={dismissPopup} hitSlop={{
        top: 10,
        bottom: 10,
        left: 10,
        right: 10
      ...
      {/* ── OSM ATTRIBUTION ──────────────────────────────────────────── */}
      {useOsmFallback && <View style={styles.osmBadge} pointerEvents="none">
          <Ionicons name="map-outline" size={11} color="#92400e" />
          <Text style={styles.osmBadgeText}>OpenStreetMap</Text>
        </View>}

      {/* ── BOTTOM DETAIL CARD ───────────────────────────────────────── */}
      {nearestGeofence && distance !== null && showDetails && <View style={styles.bottomPanel}>
          ...
          backgroundColor: accentColor
            ...
            <View style={styles.cardContent}>
              ...
              backgroundColor: accentColor + '18'
                  ...
                  <Ionicons name={isInside ? 'checkmark-circle' : 'close-circle'} size={26} color={accentColor} />
                </View>
            ...
            backgroundColor: theme.colors.border
              ...
              <View style={styles.cardGrid}>
                <CardRow label="Location" value={nearestGeofence.name} theme={theme} />
                <CardRow label="Allowed radius" value={`${nearestGeofence.radius} m`} theme={theme} />
                {!isInside && <CardRow label="Need to move" value={`${Math.round(distance - nearestGeofence.radius)} m closer`} valueColor="#ef4444" theme={theme} />}
                {locationHistory.length > 1 && <CardRow label="Trail points" value={`${locationHistory.length} recorded`} valueColor="#6366f1" theme={theme} />}
              </View>
            </View>
          </View>
        </View>}
      ...
      {/* ── MINIMISED PILL ───────────────────────────────────────────── */}
      {!showDetails && nearestGeofence && <TouchableOpacity style={[styles.miniPill, {
      backgroundColor: accentColor
    }]} onPress={() => setShowDetails(true)}>
          <Ionicons name={isInside ? 'checkmark-circle' : 'alert-circle'} size={17} color="#fff" />
