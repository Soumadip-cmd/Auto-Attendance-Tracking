      ...
      .map((geofence) => {
        ...
        const distance =
          location && Number.isFinite(latitude) && Number.isFinite(longitude)
            ? getDistance(location.latitude, location.longitude, latitude, longitude)
            : null;
    ...
    >
      ...
      {location && (
        <Card style={styles.locationCard}>
          <Text style={[styles.sectionLabel, { color: theme.colors.textSecondary }]}>Your Location</Text>
      ...
      <View style={styles.list}>
        {locations.map((geofence) => (
          <Card key={geofence._id} style={styles.locationItem}>
            <View style={styles.locationHeader}>
              <View>
                <Text style={[styles.locationName, { color: theme.colors.text }]}>{geofence.name}</Text>
              ...
              <View
                style={[
                  styles.badge,
                  {
                    backgroundColor: geofence.isInside
            ...
            <View style={styles.detailsRow}>
              ...
              {geofence.distance != null && (
                <Text style={[styles.distanceText, { color: theme.colors.text }]}>
                  {geofence.distance >= 1000
                    ? `${(geofence.distance / 1000).toFixed(2)} km`
                    : `${Math.round(geofence.distance)} m`}
                </Text>
              )}
            </View>
          </Card>
