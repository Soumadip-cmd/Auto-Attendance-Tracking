    ...
    const connect = async () => {
      try {
        ...
        unsubApproved = websocketService.on('movement-permission:approved', async data => {
          const endStr = data.endTime ? new Date(data.endTime).toLocaleTimeString([], {
            hour: '2-digit',
            minute: '2-digit'
          }) : '';
          const grantedBy = data.grantedBy ? ` by ${data.grantedBy}` : '';
          await notificationService.scheduleNotification('Movement permission granted', `Your request was approved${grantedBy}. Valid until ${endStr}. Stay within ${data.radius}m.`, {
            type: 'movement_permission_approved',
            id: data.id
          });
        });
        unsubRejected = websocketService.on('movement-permission:rejected', async data => {
          await notificationService.scheduleNotification('Movement permission rejected', data?.decisionNotes || 'Your movement permission request was not approved.', {
...
export default function RootLayout() {
  ...
  if (!fontsLoaded && !fontError) {
    // Native splash screen stays visible (preventAutoHideAsync above) until this resolves
    return null;
  }
