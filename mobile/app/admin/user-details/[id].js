...
export default function UserDetailsScreen() {
  ...
  const loadUserDetails = async () => {
    try {
      ...
      // Load user data
      const userResponse = await axios.get(`${config.API_URL}/users/${id}`, headers);
      ...
      const presentDays = attendanceResponse.data.data.filter(a => a.status === 'present').length;
      const lateDays = attendanceResponse.data.data.filter(a => a.status === 'late').length;
      const absentDays = attendanceResponse.data.data.filter(a => a.status === 'absent').length;
      setStats({
        totalDays,
        presentDays,
        lateDays,
  ...
  const handleToggleStatus = async () => {
    ...
    Alert.alert(`${newStatus === 'active' ? 'Activate' : 'Deactivate'} User`, `Are you sure you want to ${newStatus === 'active' ? 'activate' : 'deactivate'} this user?`, [{
      text: 'Cancel',
      style: 'cancel'
    }, {
      ...
      onPress: async () => {
        try {
          const headers = await getAuthHeaders();
          await axios.put(`${config.API_URL}/users/${id}`, {
            status: newStatus
          }, headers);
      ...
      <ScrollView style={styles.content} contentContainerStyle={styles.scrollContent} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
        ...
        <Card style={styles.profileCard}>
          <View style={styles.profileHeader}>
            <Avatar source={getUserAvatarUri(user) ? {
            uri: getUserAvatarUri(user)
          } : null} name={`${user.firstName} ${user.lastName}`} size={80} />
            <View style={styles.profileInfo}>
              ...
              <View style={styles.badges}>
                ...
                backgroundColor: user.role === 'admin' ? `${theme.colors.primary}20` : `${theme.colors.success}20`
              }]}>
                  <Text style={[styles.roleText, {
                  color: user.role === 'admin' ? theme.colors.primary : theme.colors.success
                }]}>
                    {user.role}
                  </Text>
                </View>
                <View style={[styles.statusBadge, {
                backgroundColor: user.status === 'active' ? `${theme.colors.success}20` : `${theme.colors.error}20`
              }]}>
        ...
        {/* Stats Card */}
        {stats && <Card style={styles.statsCard}>
        ...
        <Card style={styles.attendanceCard}>
          ...
          {attendance.length > 0 ? attendance.slice(0, 10).map(record => <View key={record._id} style={[styles.attendanceRecord, {
        ...
        <View style={styles.actions}>
          ...
          backgroundColor: user.status === 'active' ? `${theme.colors.warning}20` : `${theme.colors.success}20`,
          borderColor: user.status === 'active' ? theme.colors.warning : theme.colors.success
        }]} onPress={handleToggleStatus}>
            <Ionicons name={user.status === 'active' ? 'pause' : 'play'} size={20} color={user.status === 'active' ? theme.colors.warning : theme.colors.success} />
