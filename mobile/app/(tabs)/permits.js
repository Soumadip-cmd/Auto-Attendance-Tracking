...
export default function PermitsScreen() {
  ...
  const loadPermits = useCallback(async (silent = false) => {
    try {
      ...
      const [activeResponse, requestsResponse] = await Promise.all([movementPermissionAPI.getActivePermission(), movementPermissionAPI.getAll()]);
  ...
  const cancelPermit = permit => {
    Alert.alert('Cancel permit', 'Do you want to cancel this movement permission request?', [{
      text: 'No',
      style: 'cancel'
    }, {
      text: 'Cancel permit',
    ...
    backgroundColor: theme.colors.background
      ...
      paddingBottom: insets.bottom + 96
        ...
        <View style={styles.header}>
          ...
          <TouchableOpacity onPress={() => router.push('/movement-request')} style={[styles.addButton, {
        ...
        {loading ? <View style={styles.centerState}>
          ...
          color: theme.colors.textSecondary
            ...
            {requests.length === 0 ? <Card style={styles.emptyCard}>
            ...
            color: theme.colors.textSecondary
              ...
              </Card> : requests.map(permit => <PermitItem key={permit._id} permit={permit} actionId={actionId} onCancel={cancelPermit} />)}
          </>}
      </ScrollView>
    </View>;
}
function ActivePermitCard({
  permit
}) {
  const {
    theme
  ...
  return <Card style={[styles.activeCard, {
    borderLeftColor: permit ? meta.color : theme.colors.border
  }]}>
      <View style={styles.rowBetween}>
        <View style={styles.rowCenter}>
          <View style={[styles.bigIcon, {
          backgroundColor: permit ? meta.bg : theme.colors.background
        }]}>
      ...
      {permit?.reason && <Text style={[styles.reason, {
      color: theme.colors.text
    }]} numberOfLines={3}>
          {permit.reason}
        </Text>}
...
function PermitItem({
  ...
  return <View style={[styles.permitItem, {
    backgroundColor: theme.colors.card
  }]}>
      <View style={styles.rowBetween}>
        <View style={{
      ...
      <View style={styles.permitMeta}>
        <View style={styles.metaItem}>
          ...
          color: theme.colors.textSecondary
        }]}>{permit.radius}m</Text>
        </View>
        {permit.approvedBy && <View style={styles.metaItem}>
      ...
      {permit.decisionNotes ? <Text style={[styles.decisionNotes, {
      color: theme.colors.textSecondary
    }]}>
          {permit.decisionNotes}
        </Text> : null}

      {canCancel && <TouchableOpacity disabled={actionId === permit._id} onPress={() => onCancel(permit)} style={[styles.cancelButton, {
