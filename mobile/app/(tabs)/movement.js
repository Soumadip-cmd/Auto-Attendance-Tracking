...
export default function MovementScreen() {
  ...
  const loadTrail = useCallback(async (silent = false) => {
    ...
    try {
      ...
      setError(null);
      const now = new Date();
      const start = selectedRange.days === 0 ? new Date(now.getFullYear(), now.getMonth(), now.getDate()) : subDays(now, selectedRange.days);
    ...
    backgroundColor: theme.colors.background
      ...
      paddingBottom: insets.bottom + 96
        ...
        <View style={styles.header}>
          ...
          <TouchableOpacity onPress={() => loadTrail()} style={[styles.iconButton, {
        ...
        <View style={styles.segment}>
          {RANGES.map(item => <TouchableOpacity key={item.key} onPress={() => setRange(item.key)} style={[styles.segmentButton, {
        ...
        {loading ? <View style={styles.centerState}>
          ...
          color: theme.colors.textSecondary
            ...
            <Card style={styles.lastCard}>
              ...
              color: theme.colors.text
                  ...
                  {metrics.last ? `${metrics.last.latitude.toFixed(6)}, ${metrics.last.longitude.toFixed(6)}` : 'No location recorded yet'}
                </Text>
              </View>
            </Card>
          ...
          color: theme.colors.text
        }]}>Recent movement</Text>
            {points.length === 0 ? <Card style={styles.stateCard}>
                <Ionicons name="walk-outline" size={28} color={theme.colors.textSecondary} />
                <Text style={[styles.stateTitle, {
            color: theme.colors.text
          }]}>No movement data</Text>
                <Text style={[styles.stateText, {
            color: theme.colors.textSecondary
          }]}>
                  Start live tracking from the map screen to record your trail.
                </Text>
              </Card> : points.slice(-20).reverse().map(point => <View key={point.id || point.timestamp} style={[styles.timelineItem, {
          backgroundColor: theme.colors.card
        }]}>
                  <View style={[styles.timelineDot, {
            backgroundColor: point.violation ? '#ef4444' : '#10b981'
          }]} />
